...
public class TypeToRuleVisitor implements TypeVisitor<Rule>
{
    ...
    public static boolean NILLABLE_BASE_TYPES = Boolean.parseBoolean(System.getProperty(NILLABLE_STRINGS_PROP, "false"));
    ...
    {
        final AllOfRule typeRule = new AllOfRule(new StringTypeRule(NILLABLE_BASE_TYPES));
        registerRule(stringTypeNode, typeRule);
        if (isNotEmpty(stringTypeNode.getPattern()))
        {
            typeRule.and(new RegexValueRule(Pattern.compile(stringTypeNode.getPattern()), NILLABLE_BASE_TYPES));
        }
        ...
        if (stringTypeNode.getMaxLength() != null)
        {
            ...
            typeRule.and(new MaxLengthRule(maxLength, NILLABLE_BASE_TYPES));
        }

        if (stringTypeNode.getMinLength() != null)
        {
            Integer maxLength = stringTypeNode.getMinLength();
            typeRule.and(new MinLengthRule(maxLength, NILLABLE_BASE_TYPES));
        }
        return typeRule;
    }
    ...
    {
        return visitNumber(integerTypeDefinition, new IntegerTypeRule(CAST_STRINGS_AS_NUMBERS, NILLABLE_BASE_TYPES));
    }

    @Override
    public Rule visitNumber(NumberResolvedType numberTypeDefinition)
    {
        return visitNumber(numberTypeDefinition, new NumberTypeRule(CAST_STRINGS_AS_NUMBERS, NILLABLE_BASE_TYPES));
    }
    ...
    private Rule visitNumber(NumberResolvedType numericTypeNode, Rule numericTypeRule)
    {
        ...
        if (numericTypeNode.getMinimum() != null && numericTypeNode.getMaximum() != null)
        {
            if (numericTypeNode.getMinimum().getClass() == numericTypeNode.getMaximum().getClass())
            {
                ...
                typeRule.and(new RangeValueRule(new NumberRange(numericTypeNode.getMinimum(), numericTypeNode.getMaximum()), NILLABLE_BASE_TYPES));
            }
            else
            {

                typeRule.and(new RangeValueRule(new NumberRange(numericTypeNode.getMinimum().doubleValue(), numericTypeNode.getMaximum().doubleValue()), NILLABLE_BASE_TYPES));
            }
        }
        else if (numericTypeNode.getMinimum() != null)
        {
            typeRule.and(new MinimumValueRule(numericTypeNode.getMinimum(), NILLABLE_BASE_TYPES));
        }
        else if (numericTypeNode.getMaximum() != null)
        {
            typeRule.and(new MaximumValueRule(numericTypeNode.getMaximum(), NILLABLE_BASE_TYPES));
        }
        if (numericTypeNode.getMultiple() != null)
        {
            typeRule.and(new DivisorValueRule(numericTypeNode.getMultiple(), CAST_STRINGS_AS_NUMBERS, NILLABLE_BASE_TYPES));
        }
        if (numericTypeNode.getFormat() != null)
        {
            typeRule.and(new FormatValueRule(numericTypeNode.getFormat(), NILLABLE_BASE_TYPES));
        }
